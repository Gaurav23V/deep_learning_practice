...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]
    D = np.prod(x.shape[1:])  # Calculate D = d_1 * ... * d_k
    x_reshaped = x.reshape(N, D)  # Reshape x to (N, D)
    out = x_reshaped.dot(w) + b  # Perform the affine transformation
...
def affine_backward(dout, cache):
    ...
    dx = dout.dot(w.T)  # Calculate dx
    dx = dx.reshape(*x.shape)  # Reshape dx to the original shape of x
    dw = x_reshaped.T.dot(dout)  # Calculate dw
    db = np.sum(dout, axis=0)  # Calculate db
...
def relu_forward(x):
    ...
    out = np.maximum(0, x)  # Apply the ReLU function
...
def relu_backward(dout, cache):
    ...
    x = cache
    dx = dout * (x > 0)  # Backward pass for ReLU
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        # Store the running mean and variance in the bn_param dictionary
...
def batchnorm_backward(dout, cache):
    ...
    dmean = np.sum(dx_hat * -1 / np.sqrt(sample_var + eps), axis = 0) + dvar * np.sum(-2 * (x - sample_mean), axsi = 0) / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1. / N) * (1. / np.sqrt(sample_var + eps)) * (N * dx_hat - np.sum(dx_hat, axis = 0) - x_hat * np.sum(dx_hat * x_hat, axis = 0))
...
def layernorm_backward(dout, cache):
    ...
    dmean = np.sum(dx_hat * -1 / np.sqrt(sample_var + eps), axis=1, keepdims=True) + dvar * np.sum(-2 * (x - sample_mean), axis=1, keepdims=True) / D
